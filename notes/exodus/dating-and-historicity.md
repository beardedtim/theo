...
is well attested and likely the historical kernel later traditions grew
from, which is why a "late date" (13th century, Ramesses II) remains the
more commonly cited option when a historical core is assumed at all -- but
even that is a minority-within-a-minority position; a large share of
current scholarship treats Exodus as a foundational/etiological narrative
composed centuries later (exilic/post-exilic), not a memory of a specific
13th-or-15th-century event.

Net: treat "when did the Exodus happen" as a live, unsettled, minority-vs-
majority scholarly question, not a solved chronology problem with a single
correct answer that conservative harmonization implies.
